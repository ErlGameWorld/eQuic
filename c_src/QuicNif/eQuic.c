...
void ConnResDown(ErlNifEnv* caller_env, void* obj, ErlNifPid* pid, ErlNifMonitor* mon) {
	...
	ConnRes* ObjIns = (ConnRes*)obj;
	if (enif_compare_pids(ObjIns->OwnerPid, pid)) {
...
QUIC_STATUS SrvConnHandler(HQUIC Connection, void* Context, QUIC_CONNECTION_EVENT* Event) {
	// ConnRes* ConnObjIns = (ConnRes*)Context;

	switch (Event->Type) {
	case QUIC_CONNECTION_EVENT_CONNECTED:				// 握手完毕
		break;
...
QUIC_STATUS ListenerHandler(HQUIC Listener, void* Context, QUIC_LISTENER_EVENT* Event) {
	...
	case QUIC_LISTENER_EVENT_NEW_CONNECTION: {
		...
		MsQuic->SetCallbackHandler(Event->NEW_CONNECTION.Connection, (void*)SrvConnHandler, ObjIns);
